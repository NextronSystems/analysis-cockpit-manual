...
SIGMA matches generated by the Management Center's endpoint agents.

It can be used in an environment where scans results can be
automatically collected from Management Centers or environments in
which THOR is executed by scripts or any other
...
configurable connector. If the Management Center's Bifrost\_2 Service is
used to collect suspicious samples, the Analysis Cockpit can submit the
samples to various Sandbox systems and include the report in the
graphical frontend.
